...
def on_connect(client, userdata, flags, rc):
   ...
   # client.publish("server/assign_id","Device_Type: beagleboard")
    client.subscribe("client/beagleboard")
    client.subscribe("client/jetson_up")
    client.subscribe("beagle_1/rtt")
...
def on_message(client, userdata, msg):
    ...
    global rtt
    global jetson_up
    if(msg.topic=="client/beagleboard"):
...
client = mqtt.Client("Beagleboard")
# Callback functions!
...
while(True):
    ...
    while(input<10):
        if(input % 2 ==0):
            ...
            input = input+1
            print("Output from beagleboard  is: "+str(input))
        else:
            if jetson_up==1:
                starting_time = time.time()
                publish_value = client.publish("server/rtt_beagle_1","1",qos=2)
                ...
                starting_time = time.time()
                published_value=client.publish("server/beagleboard",str(input),qos=2)
                ...
                input = input+1
                print("Calculating data localy")
                print("Output from beagleboard pi is: "+str(input))
        if client.is_connected() == False:
            client.reconnect()
